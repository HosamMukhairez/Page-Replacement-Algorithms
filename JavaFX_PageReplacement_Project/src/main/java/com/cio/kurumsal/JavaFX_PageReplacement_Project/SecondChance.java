...
class SecondChance {
    ...
    public String printHitsAndFaults(String reference_string[], int frames) {
        int pointer, i, l, x, pf;
        ...
        //String str[] = reference_string.split(" ");
        //get the length of array
        l = reference_string.length;

        for (i = 0; i < l; i++) {

            x = Integer.parseInt(reference_string[i]);
            ...
            if (!findAndUpdate(x, arr, second_chance, frames)) {
                ...
                output += " Updated page " + x + "\n";
                //Update page faults
                pf++;
            }
        }
        output += "Total page faults were " + pf + "\n";
        System.out.println("Total page faults were " + pf);
        return output;
    }
}
